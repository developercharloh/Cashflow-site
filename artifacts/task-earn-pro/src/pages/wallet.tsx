...
import {
  ...
  Plus, Banknote, CreditCard, Landmark
} from "lucide-react";
...
export default function WalletPage() {
  ...
  const { data: banks } = useGetBanks({ query: { queryKey: ["getBanks"], enabled: modal === "withdraw_paystack" } });
  ...
  useEffect(() => {
    ...
    if (ref) {
      ...
      fetch(`/api/paystack/deposit/verify/${ref}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
        .then(r => r.json())
        .then(d => {
          if (d.message) {
            toast({ title: "Deposit Successful! 🎉", description: d.message });
  ...
  const handlePaystackWithdraw = () => {
    ...
    paystackWithdrawMutation.mutate({ data: { amount: amt, bankCode, accountNumber, accountName } }, {
      onSuccess: () => {
        toast({ title: "Withdrawal Initiated", description: "Your transfer is being processed by Paystack." });
        setModal(null);
  ...
  return (
    <div className="px-4 pt-4 space-y-5">
      ...
      <div className="rounded-2xl p-5 text-white" style={{ background: "linear-gradient(135deg, #0f2027 0%, #203a43 50%, #2c5364 100%)" }}>
        ...
        <div className="grid grid-cols-2 gap-3">
          <button
            ...
            className="flex items-center justify-center gap-2 py-3 rounded-xl font-bold text-sm transition-opacity hover:opacity-90"
      ...
      <Dialog open={modal === "deposit"} onOpenChange={(o) => !o && setModal(null)}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>Securely deposit funds using your card, bank, or mobile money.</DialogDescription>
          </DialogHeader>
          <div className="space-y-4 pt-2">
            ...
            <Button
              className="w-full" onClick={handleDeposit}
      ...
      <Dialog open={modal === "withdraw_paystack"} onOpenChange={(o) => !o && setModal(null)}>
        <DialogContent className="max-w-sm max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Withdraw directly to your bank via Paystack. Minimum $5.</DialogDescription>
          </DialogHeader>
          <div className="space-y-4 pt-2">
            <div>
              <Label>Amount (USD)</Label>
      ...
      {/* ── MANUAL WITHDRAW MODAL (M-Pesa / PayPal) ───────────────────────── */}
      <Dialog open={modal === "withdraw_manual"} onOpenChange={(o) => !o && setModal(null)}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>Request via M-Pesa, Airtel Money, or PayPal. Processed within 24h. Minimum $5.</DialogDescription>
          </DialogHeader>
          <div className="space-y-4 pt-2">
            <div>
              <Label>Amount (USD)</Label>
